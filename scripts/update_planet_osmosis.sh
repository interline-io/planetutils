...
# 1. https://github.com/artemp/MapQuest-Render-Stack/blob/master/scripts/minutely_update.sh (LGPL)
# 2. https://github.com/valhalla/docker/blob/master/scripts/minutely_update.sh (MIT)
...
initialize() {
  ...
  if [ ! -e $WORKDIR_OSM/state.txt ]; then
    ...
    aws s3 cp s3://${URL} ${BASE_DIR}/${PLANET_FILE}
    ...
    aws s3 cp s3://${URL}.md5 ${BASE_DIR}/${PLANET_FILE}.md5
    if [ $? -ne 0 ]; then
